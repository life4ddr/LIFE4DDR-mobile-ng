...
import com.perrigogames.life4ddr.nextgen.feature.trials.manager.TrialSettings.Companion.KEY_TRIAL_LAST_SYNC
...
interface TrialSettings {
    ...
    fun setLastSyncTime(time: Instant)
    ...
    companion object {
        ...
        const val KEY_TRIAL_LAST_SYNC = "KEY_TRIAL_LAST_SYNC"
    }
}
...
class DefaultTrialSettings : SettingsManager(), TrialSettings {
    ...
    override val lastSyncTime: Instant?
        get() = basicSettings.getStringOrNull(KEY_TRIAL_LAST_SYNC)?.let { Instant.parse(it) }

    override fun setLastSyncTime(time: Instant) {
        basicSettings.putString(KEY_TRIAL_LAST_SYNC, time.toString())
    }

    override fun clearLastSyncTime() {
        basicSettings.remove(KEY_TRIAL_LAST_SYNC)
    }
}
